...
1. 使用用户端首页查询酒店，展示城市、日期、标签和列表筛选。
2. 进入酒店详情页，展示房型价格升序、库存紧张、售罄禁用、收藏和预订。
3. 使用商户账号登录后台，编辑酒店信息或房型库存。
4. 使用管理员账号登录后台，审核、发布、下线或恢复酒店。
5. 回到用户端观察 SSE 实时刷新效果。

## 说明

本项目适合作为前端训练营或全栈课程大作业展示项目，重点体现真实数据流、真实状态变化、角色权限、管理审核流程和移动端交互体验。
