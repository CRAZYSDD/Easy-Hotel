...
1. 在用户端首页选择城市、日期和热门偏好，进入酒店列表页。
2. 在列表页展开筛选与排序，展示多条件筛选、无限滚动和售罄酒店沉底。
3. 进入酒店详情页，展示房型价格升序、售罄房型沉底、收藏和模拟预订。
4. 进入我的收藏页，展示收藏状态同步和售罄状态同步。
5. 使用商户账号登录后台，编辑酒店信息或房型库存。
6. 使用管理员账号登录后台，完成审核、发布、下线和恢复操作。
7. 回到用户端观察 SSE 实时刷新效果。
